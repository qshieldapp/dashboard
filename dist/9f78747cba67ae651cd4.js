...
async function deployF2(){
  ...
  var chainId = 1287;
  var cid = '0x507';
  var chain = 'Moonbase Alpha Testnet';
  var name = 'Moonbase Alpha Testnet';
  var symbol = 'DEV';
  var rpc = "https://moonbase.unitedbloc.com";
  ...
  const factory = new ethers.ContractFactory(abi.abi, bytecode, signer);
  const contract = await factory.deploy('0x0D6a25f8dBfDE877CedA48C73EB9CEB3F39bD238', 5);
  await contract.waitForDeployment();
  console.log(contract.target);
  //console.log(contract.target);
  document.getElementById('res_sct2').style.display = 'block';
